...
func (f *Flow) Execute() error {
	...
	for _, t := range f.ErrornousTasks {
		e += sep + fmt.Sprintf("'%s' returned '%s'", t, t.Error)
		sep = ", "
	}
	return errors.New(e)
}
...
func (f *Flow) handleFlow() {
	for len(f.ActiveTasks) > 0 {
		...
		if t.Error != nil {
			f.errorf("An error occurred while executing %s: %s", t, t.Error)
...
func (f *Flow) startTask(task *Task) {
	...
	go func() {
		if !task.Skip {
			...
			task.Error = utils.Retry(f.Logger, task.RetryDuration, utils.RetryFunc(task.Function))
		} else {
