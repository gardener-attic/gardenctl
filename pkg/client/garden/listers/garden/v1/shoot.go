...
type ShootLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Shoot, err error)
...
// List lists all Shoots in the indexer.
func (s *shootLister) List(selector labels.Selector) (ret []*v1.Shoot, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Shoot))
	})
	return ret, err
}
...
type ShootNamespaceLister interface {
	...
	Get(name string) (*v1.Shoot, error)
...
// List lists all Shoots in the indexer for a given namespace.
func (s shootNamespaceLister) List(selector labels.Selector) (ret []*v1.Shoot, err error) {
...
// Get retrieves the Shoot from the indexer for a given namespace and name.
func (s shootNamespaceLister) Get(name string) (*v1.Shoot, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("shoot"), name)
	}
	return obj.(*v1.Shoot), nil
}
