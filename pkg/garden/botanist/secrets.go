...
func (b *Botanist) DeploySecrets() error {
	...
	secrets, err := b.
		K8sSeedClient.
		ListSecrets(b.ShootNamespace, metav1.ListOptions{})
	if err != nil {
		return err
	}
	...
	if val, ok := secretsMap[name]; ok {
		...
		b.Secrets[name], err = b.
			K8sSeedClient.
			CreateSecret(b.ShootNamespace, name, corev1.SecretTypeOpaque, data, false)
		if err != nil {
			return err
		}
	}
	...
	if val, ok := secretsMap[name]; ok {
		b.Secrets[name] = val
		csv := strings.Split(string(val.Data["basic_auth.csv"]), ",")
		basicAuthData = map[string]string{
			"username": csv[1],
			"password": csv[0],
		}
		...
		data = map[string][]byte{
			"basic_auth.csv": []byte(fmt.Sprintf("%s,%s,%s,system:masters", basicAuthPassword, basicAuthUsername, basicAuthUsername)),
		}
		b.Secrets[name], err = b.
			K8sSeedClient.
			CreateSecret(b.ShootNamespace, name, corev1.SecretTypeOpaque, data, false)
	...
	// Third we create the infrastructure secret which contains the credentials for the cloud provider.
	_, err = b.
		K8sSeedClient.
		CreateSecret(b.ShootNamespace, "infrastructure", corev1.SecretTypeOpaque, b.Secrets["infrastructure"].Data, true)
	if err != nil {
		return err
	}
...
func (b *Botanist) createRSASecret(secret RSASecret) (*corev1.Secret, error) {
	...
	return b.
		K8sSeedClient.
		CreateSecret(b.ShootNamespace, secret.Name, corev1.SecretTypeOpaque, data, false)
}
...
func (b *Botanist) createTLSSecret(secret TLSSecret, CACertificateTemplate *x509.Certificate, CAPrivateKey *rsa.PrivateKey) (*corev1.Secret, error) {
	...
	return b.
		K8sSeedClient.
		CreateSecret(b.ShootNamespace, secret.Name, corev1.SecretTypeTLS, data, false)
}
...
func (b *Botanist) createControlPlaneSecret(secret ControlPlaneSecret, CACertificatePEM []byte, CACertificateTemplate *x509.Certificate, CAPrivateKey *rsa.PrivateKey, basicAuthData map[string]string) (*corev1.Secret, error) {
	...
	if secret.KubeconfigRequired {
		...
		kubeconfig, err := generateKubeconfig(b.ShootNamespace, apiServerURL, utils.EncodeBase64(CACertificatePEM), utils.EncodeBase64(certificatePEM), utils.EncodeBase64(privateKeyPEM), basicAuthUser, basicAuthPass)
		if err != nil {
			return nil, err
		}
...
func (b *Botanist) computeAPIServerURL(runsInSeed, useInternalClusterDomain bool) string {
	...
	dnsKind := b.Shoot.Spec.DNS.Kind
	if dnsKind == gardenv1.DNSUnmanaged || (dnsKind != gardenv1.DNSUnmanaged && useInternalClusterDomain) {
		return b.InternalClusterDomain
	}
	return b.ExternalClusterDomain
}
...
func (b *Botanist) computeSecretsCheckSums() error {
	for name, secret := range b.Secrets {
		...
		b.CheckSums[name] = utils.ComputeSHA256Sum(jsonString)
	}
	return nil
}
...
func (b *Botanist) generateSecrets() ([]interface{}, error) {
	...
	apiServerCertDNSNames := []string{
		fmt.Sprintf("kube-apiserver.%s", b.ShootNamespace),
		fmt.Sprintf("kube-apiserver.%s.svc", b.ShootNamespace),
		fmt.Sprintf("kube-apiserver.%s.svc.cluster.local", b.ShootNamespace),
		"kube-apiserver",
		"kubernetes",
		"kubernetes.default",
		"kubernetes.default.svc",
		"kubernetes.default.svc.cluster.local",
		b.InternalClusterDomain,
	}
	if b.ExternalClusterDomain != "" {
		apiServerCertDNSNames = append(apiServerCertDNSNames, b.Shoot.Spec.DNS.Domain, b.ExternalClusterDomain)
	}
	...
	secretList := []interface{}{
		...
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				...
				IPAddresses: []net.IP{
					...
					net.ParseIP(garden.ComputeClusterIP(b.Shoot.Spec.Networks.Services, 1)),
				},
				IsServerCert: true,
			},
		...
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				...
				CommonName:   "sapcloud:addons:addon-manager",
		...
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				...
				CommonName:   "sapcloud:monitoring:auto-node-repair",
				Organization: []string{"sapcloud:monitoring"},
				DNSNames:     nil,
				IPAddresses:  nil,
				IsServerCert: false,
			},
		...
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				...
				CommonName:   "sapcloud:monitoring:kube-state-metrics",
				Organization: []string{"sapcloud:monitoring"},
				DNSNames:     nil,
				IPAddresses:  nil,
				IsServerCert: false,
		...
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				...
				CommonName:   "sapcloud:monitoring:prometheus",
				Organization: []string{"sapcloud:monitoring"},
				DNSNames:     nil,
				IPAddresses:  nil,
				IsServerCert: false,
		...
		// Secret definition for gardenctl
		ControlPlaneSecret{
			TLSSecret: TLSSecret{
				Secret: Secret{
					Name: "gardenctl",
				},
				CommonName:   "gardenctl",
				Organization: []string{"system:masters"},
				DNSNames:     nil,
				IPAddresses:  nil,
				IsServerCert: false,
		...
		TLSSecret{
			...
			Organization: []string{"sapcloud:monitoring:ingress"},
	...
	if b.Shoot.Spec.Addons.Monocular.Enabled {
