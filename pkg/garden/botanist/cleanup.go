...
func (b *Botanist) CleanupNamespaces() error {
	return b.
		K8sShootClient.
		CleanupNamespaces(namespaceExceptions)
}
...
func (b *Botanist) CheckNamespaceCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckNamespaceCleanup(namespaceExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupServices() error {
	return b.
		K8sShootClient.
		CleanupServices(serviceExceptions)
}
...
func (b *Botanist) CheckServiceCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckServiceCleanup(serviceExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupStatefulSets() error {
	return b.
		K8sShootClient.
		CleanupStatefulSets(workloadExceptions)
}
...
func (b *Botanist) CheckStatefulSetCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckStatefulSetCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupDeployments() error {
	return b.
		K8sShootClient.
		CleanupDeployments(workloadExceptions)
}
...
func (b *Botanist) CheckDeploymentCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckDeploymentCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupReplicationControllers() error {
	return b.
		K8sShootClient.
		CleanupReplicationControllers(workloadExceptions)
}
...
func (b *Botanist) CheckReplicationControllerCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckReplicationControllerCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupReplicaSets() error {
	return b.
		K8sShootClient.
		CleanupReplicaSets(workloadExceptions)
}
...
func (b *Botanist) CheckReplicaSetCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckReplicaSetCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupJobs() error {
	return b.
		K8sShootClient.
		CleanupJobs(workloadExceptions)
}
...
func (b *Botanist) CheckJobCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckJobCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupPods() error {
	return b.
		K8sShootClient.
		CleanupPods(workloadExceptions)
}
...
func (b *Botanist) CheckPodCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckPodCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupDaemonSets() error {
	return b.
		K8sShootClient.
		CleanupDaemonSets(workloadExceptions)
}
...
func (b *Botanist) CheckDaemonSetCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckDaemonSetCleanup(workloadExceptions)
	if err != nil {
		if !finished {
...
func (b *Botanist) CleanupPersistentVolumeClaims() error {
	return b.
		K8sShootClient.
		CleanupPersistentVolumeClaims(nil)
}
...
func (b *Botanist) CheckPersistentVolumeClaimCleanup() error {
	finished, err := b.
		K8sShootClient.
		CheckPersistentVolumeClaimCleanup(nil)
	if err != nil {
		if !finished {
